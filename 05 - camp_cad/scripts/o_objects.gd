...
@onready var connection_points_container: Node2D = $connection_points_container
...
func _process(_delta: float) -> void:
	if debug:
		size_m = Global.to_m(size)
		pos_m = Global.to_m(position)
...
func set_properties(cat: String, obj_name: String, template_data: Dictionary, num_sections: int):
...
signal object_pressed(new_object: Array)
func _on_button_pressed() -> void:
	emit_signal("object_pressed", [self])
	toggle_expand()
