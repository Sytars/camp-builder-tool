...
func save_data():
	...
	var camp_cad = root.get_node_or_null("MarginContainer/HBoxContainer/Calculator Window/Panel/MarginContainer/Camp Cad2/Scripts/Utils")

	var scenes = [ohm_law_calc_node, settings, camp_cad]
	
	if Global.has_method("_get_save_data"):
		data_to_save["/root/Global"] = Global._get_save_data()
	
	for scene in scenes:
		if scene:
			if scene.has_method("_get_save_data"):
				var node_path_str = str(scene.get_path()) # Use its full path as the key
				data_to_save[node_path_str] = scene._get_save_data()
			else:
