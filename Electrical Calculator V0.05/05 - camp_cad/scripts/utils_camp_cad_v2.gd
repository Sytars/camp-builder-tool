...
@onready var selector = $"../../Panel/VBoxContainer/Canvas/Origin/Selector"
...
func _apply_load_data(_loaded_data: Dictionary):
	pass
...
func update_selector(selection):
	if not init_pos:
		init_pos = origin.get_local_mouse_position()
	var mouse_pos = origin.get_local_mouse_position()
	# Horizontal
	if init_pos.x > mouse_pos.x:
		selection.color = Color(0, 1, 0, 0.1)
		selection.position.x = mouse_pos.x
		selection.size.x = init_pos.x - mouse_pos.x
	else:
		selection.color = Color(0, 0, 1, 0.1)
		selection.position.x = init_pos.x
		selection.size.x = mouse_pos.x - init_pos.x
	# Vertical
	if init_pos.y > mouse_pos.y:
		selection.position.y = mouse_pos.y
		selection.size.y = init_pos.y - mouse_pos.y
	else:
		selection.position.y = init_pos.y
		selection.size.y = mouse_pos.y - init_pos.y
	return {
		"position": selection.position,# - origin.position, # Substracting origin pos because of panning. not panned origin pos = (0, 0)
		"size": selection.size
	}
