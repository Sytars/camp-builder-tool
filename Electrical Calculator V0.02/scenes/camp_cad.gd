	...
	if event is InputEventMouseButton:
		if event.button_index == MOUSE_BUTTON_LEFT:
			if dragging and not event.pressed:
				...
				if tents_in_selection:
					print("Objects: ", tents_in_selection)
				select(tents_in_selection)
...
@onready var example_button = $"Panel/VBoxContainer/Panel/VBoxContainer/Top Tabs Panel/Top Tabs/New Object"

func select(objects: Array):
	#new_object.grab_focus()
	for obj in objects:
		for obj_data in object_data:
			if object_data[obj_data].has(obj):
				object_data[obj_data][obj]["obj"]
...
func pan(event) -> void:
	if event is InputEventMouseButton:
		if event.button_index == MOUSE_BUTTON_MIDDLE:
			if event.pressed:
				if is_within_bounds(event.position):
					is_panning = true
...
func new_object(type, obj):
	var new = objects.new_object(type, obj)
	new["name"] += str(len(object_data[type]))
	new["obj"].position = new["position"]
	new["obj"].size = new["size"]
	new["obj"].text = new["name"]
	new["obj"].theme = load("res://themes/Tents.tres")
	new["obj"].pressed.connect(Callable(self, "_on_button_pressed").bind(new["obj"]))
	origin.add_child(new["obj"])
	placing_object = true
	current_object = new

func _on_button_pressed(obj):
	selection = []
	print(obj)
	for cat in object_data:
		for obj_data in object_data[cat]:
			if obj == object_data[cat][obj_data]["obj"]:
				print("found a match!")
				print(cat, "/", obj_data, '/"obj"')
				selection.append(object_data[cat][obj_data])
				Tabs.set_state(edit_tab)
